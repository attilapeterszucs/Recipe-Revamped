                  ...
                  <div className="space-y-4">
                    <div>
                      <h4 className="text-lg font-semibold text-gray-800 flex items-center">
                        ☪️ Religious Dietary Requirements
                      </h4>
                      <p className="text-gray-600 text-sm mt-1">
                        Select religious dietary laws you follow for appropriate recipe recommendations.
                      </p>
                    </div>

                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                      {dietaryOptions.filter(option => option.category === 'religious').map((option) => {
                        const isSelected = settings.religiousRequirements?.includes(option.name) || false;
                        return (
                          <label
                            key={option.name}
                            className={`relative flex items-start p-4 rounded-lg border-2 cursor-pointer transition-all duration-200 ${
                              isSelected
                                ...
                                : 'bg-indigo-50 border-indigo-200 hover:bg-indigo-100 border-dashed'
                            }`}
                          >
                            <div className="flex items-center h-5">
                              <div className="relative">
                                <input
                                  ...
                                  checked={isSelected}
                                  onChange={(e) => {
                                    const currentRequirements = settings.religiousRequirements || [];
                                    const newRequirements = e.target.checked
                                      ? [...currentRequirements, option.name]
                                      : currentRequirements.filter(r => r !== option.name);
                                    updateSetting('religiousRequirements', newRequirements);
                                  }}
                                  className="sr-only"
                                />
                                <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all duration-200 cursor-pointer ${
                    ...
                    {settings.religiousRequirements && settings.religiousRequirements.length > 0 && (
                      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 rounded-xl p-6">
                        <div className="flex items-center mb-3">
                          <Shield className="w-5 h-5 text-indigo-600 mr-2" />
                          <h4 className="font-semibold text-indigo-900">Your Religious Requirements</h4>
                        </div>
                        <p className="text-indigo-800 text-sm mb-4">
                          Recipes will be filtered to comply with these {settings.religiousRequirements.length} religious dietary law(s):
                        </p>
                        <div className="flex flex-wrap gap-2">
                          {settings.religiousRequirements.map((requirement) => {
                            const requirementData = dietaryOptions.find(opt => opt.name === requirement);
                            return (
                              <span
                                key={requirement}
                                className="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium bg-white border border-indigo-200 text-indigo-800 shadow-sm"
                              >
                                <span className="mr-1">{requirementData?.icon}</span>
                                {requirement}
                                <button
                                  onClick={() => {
                                    const newRequirements = settings.religiousRequirements?.filter(r => r !== requirement) || [];
                                    updateSetting('religiousRequirements', newRequirements);
                                  }}
