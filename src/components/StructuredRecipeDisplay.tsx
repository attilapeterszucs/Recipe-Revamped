...
export const StructuredRecipeDisplay: React.FC<StructuredRecipeDisplayProps> = ({ 
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <p className="text-center text-gray-600 mt-4">Creating your recipe...</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div className="relative h-40 sm:h-48 overflow-hidden">
          ...
          {onSave && (
            <button
              ...
              className={`absolute top-3 right-3 backdrop-blur-sm text-white px-3 py-2 rounded-lg transition-all duration-200 disabled:cursor-not-allowed z-10 flex items-center space-x-2 ${
                saving 
            ...
            >
              {saving ? (
                ...
                <>
                  <Save className={`h-4 w-4 transition-colors ${recipeSaved ? 'text-green-400' : 'text-white'}`} />
      ...
      <div className="p-6">
        {/* Time & Dietary Info */}
        <div className={`grid grid-cols-1 md:grid-cols-2 ${recipeData.healthConditions && recipeData.healthConditions.length > 0 ? 'lg:grid-cols-4' : 'lg:grid-cols-3'} gap-4 mb-6`}>
          <div className="bg-blue-50 p-4 rounded-lg text-center">
            <div className="font-semibold text-blue-700">Prep Time</div>
            <div className="text-2xl font-bold text-blue-800">{recipeData.prepTime}</div>
          </div>
          <div className="bg-orange-50 p-4 rounded-lg text-center">
            <div className="font-semibold text-orange-700">Cook Time</div>
            <div className="text-2xl font-bold text-orange-800">{recipeData.cookTime}</div>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg text-center">
            <div className="font-semibold text-purple-700">Dietary</div>
            <div className="text-sm font-medium text-purple-800">{recipeData.dietaryRequirements}</div>
          </div>
          {recipeData.healthConditions && recipeData.healthConditions.length > 0 && (
            <div className="bg-pink-50 p-4 rounded-lg text-center">
              <div className="font-semibold text-pink-700">Health Conditions</div>
        ...
        <div className="bg-gray-50 p-4 rounded-lg mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
            <div className="text-center">
              <div className="font-bold text-lg text-gray-900">{recipeData.nutrition.calories}</div>
              <div className="text-xs text-gray-600">Calories</div>
            </div>
            <div className="text-center">
              <div className="font-bold text-lg text-gray-900">{recipeData.nutrition.protein}g</div>
              <div className="text-xs text-gray-600">Protein</div>
            </div>
            <div className="text-center">
              <div className="font-bold text-lg text-gray-900">{recipeData.nutrition.carbohydrates}g</div>
              <div className="text-xs text-gray-600">Carbs</div>
            </div>
            <div className="text-center">
              <div className="font-bold text-lg text-gray-900">{recipeData.nutrition.fat}g</div>
              <div className="text-xs text-gray-600">Fat</div>
            </div>
            <div className="text-center">
              <div className="font-bold text-lg text-gray-900">{recipeData.nutrition.fiber}g</div>
              <div className="text-xs text-gray-600">Fiber</div>
            </div>
            <div className="text-center">
              <div className="font-bold text-lg text-gray-900">{recipeData.nutrition.sodium}mg</div>
